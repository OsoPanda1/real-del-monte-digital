 ...
 * - Ciclo de vida explícito (draft, active, deprecated).
...
/**
 * Atributos de confiabilidad y esperados de la célula.
 */
...
export interface KnowledgeCell {
  ...
  /**
   * Plano de experiencia con el que se relaciona (turismo, institucional, técnico).
   */
...
export interface CellExecutionResult<TOutput = unknown> {
  ...
  /**
   * Registro de warnings no fatales (ej: degradación de calidad visual).
   */
  warnings?: string[];

  /**
   * ID de trace para asociar a otros sistemas (MSR, BookPI, Event Bus).
   */
  traceId?: string;
}
